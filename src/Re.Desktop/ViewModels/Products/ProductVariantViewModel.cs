...
public partial class ProductVariantViewModel : ObservableObject
{
    ...
    { _api = api; _dialog = dialog; ProductId = product.Id; ProductName = product.Name; SalePrice = product.SalePrice; _ = LoadAsync(); }
    partial void OnSelectedVariantChanged(ProductVariantResponse? value)
    { if (value is null) return; Code=value.Code; Color=value.Color??""; Size=value.Size??""; Attribute1=value.Attribute1??""; Attribute2=value.Attribute2??""; SalePrice=value.SalePrice; }
    ...
    [RelayCommand] private void NewVariant() { SelectedVariant=null; Code=""; Color=""; Size=""; Attribute1=""; Attribute2=""; }
    [RelayCommand] private async Task SaveAsync()
    {
        if(string.IsNullOrWhiteSpace(Code)){_dialog.Error("Varyant kodu zorunludur.");return;}
        var body=new SaveProductVariantRequest(Code,Color,Size,Attribute1,Attribute2,SalePrice);
        ...
        if(x is null){_dialog.Error("Varyant kaydedilemedi. Kod benzersiz olmalıdır.");return;} NewVariant(); await LoadAsync();
    }
    [RelayCommand] private async Task DeleteAsync()
    { if(SelectedVariant is null)return; if(!_dialog.Confirm("Varyant pasife alınacak. Devam edilsin mi?"))return; await _api.DeleteAsync($"api/products/{ProductId}/variants/{SelectedVariant.Id}"); await LoadAsync(); }
}
