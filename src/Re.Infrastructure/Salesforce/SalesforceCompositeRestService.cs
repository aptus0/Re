...
public sealed class SalesforceCompositeRestService : ISalesforceCompositeRestService
{
    public async Task<CompositeResult> ExecuteCompositePayloadAsync(List<CompositeSubRequest> requests, bool allOrNone = true, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return new CompositeResult(true, requests.Count, $"Composite REST API isteği {requests.Count} alt istekle atomik olarak tamamlandı.", DateTime.UtcNow);
        }
        catch (Exception ex)
        {
            return new CompositeResult(false, 0, $"Composite REST API hatası: {ex.Message}", DateTime.UtcNow);
        }
    }
